---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/bmt
- monster/cr/3
- monster/size/medium
- monster/type/humanoid/minotaur
- monster/type/humanoid/warlock
aliases: ["Brusipha"]
---
# Brusipha
*Source: The Book of Many Things p. 127*  

```ad-statblock
title: Brusipha
![](https://raw.githubusercontent.com/5etools-mirror-2/5etools-img/main/bestiary/tokens/BMT/Brusipha.webp#token)
*Medium humanoid (minotaur, warlock), Chaotic Evil*

- **Armor Class** 14 (natural armor)
- **Hit Points** 90 (`12d8 + 36`)
- **Speed** 40 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|18 (+4)|11 (+0)|16 (+3)|12 (+1)|16 (+3)|16 (+3)|

- **Proficiency Bonus** +2
- **Saving Throws** Wisdom +5, Charisma +5
- **Skills** Deception +5, Perception +7, Religion +3
- **Senses** darkvision 60 ft., passive Perception 17
- **Languages** Abyssal, Common
- **Challenge** 3

## Traits

***Demonic Ritual.*** Brusipha can spend 3 hours performing a ritual that summons `1d3 + 1` barlguras or 1 hezrou. She must sacrifice a Medium or larger living creature to Baphomet during this ritual, and the ritual can be performed only at night. The demons vanish at dawn.

***Labyrinthine Recall.*** Brusipha can perfectly recall any path she has traveled.

***Spellcasting.*** Brusipha casts one of the following spells, using Charisma as the spellcasting ability (spell save DC 13):

**At will**: [Prestidigitation](5E2014官方资源/spells/prestidigitation.md), [Speak with Animals](5E2014官方资源/spells/speak-with-animals.md)

**1/day each**: [Bestow Curse](5E2014官方资源/spells/bestow-curse.md), [Command](5E2014官方资源/spells/command.md), [Crown of Madness](5E2014官方资源/spells/crown-of-madness.md)

## Actions

***Multiattack.*** Brusipha makes two Eldritch Blast attacks.

***Eldritch Blast.*** *Ranged Spell Attack:* `+5` to hit, range 120 ft., one target. *Hit:* 8 (`1d10 + 3`) force damage.

***Gore.*** *Melee Weapon Attack:* `+6` to hit, reach 5 ft., one target. *Hit:* 8 (`1d8 + 4`) piercing damage plus 4 (`1d8`) necrotic damage. If Brusipha moved at least 10 feet straight toward the target immediately before she hit, the target takes an extra 4 (`1d8`) piercing damage, and if the target is a creature, it must succeed on a DC 15 Strength saving throw or be pushed up to 10 feet from Brusipha and have the [prone](5E2014官方资源/规则/conditions.md#prone) condition.

***Incite the Hunters (Recharges after a Short or Long Rest).*** Brusipha allows each ally within 30 feet of herself that has the Unerring Tracker trait to make one weapon attack as a reaction against the target of that ally's Unerring Tracker.

## Bonus Actions

***Unerring Tracker.*** Brusipha magically creates a psychic link with one creature she can see. For the next hour, Brusipha knows the current distance and direction to the target if it is on the same plane of existence. The link ends if Brusipha has the [incapacitated](5E2014官方资源/规则/conditions.md#incapacitated) condition or uses this ability on a different target.

## Reactions

***Baphomet's Blessing.*** When Brusipha reduces a hostile creature to 0 hit points, she gains 8 temporary hit points.
```
^statblock