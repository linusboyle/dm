...
![](https://raw.githubusercontent.com/5etools-mirror-2/5etools-img/main/bestiary/tokens/BMT/Minotaur%20Infiltrator.webp#token)
...
***Gore.*** *Melee Weapon Attack:* `+6` to hit, reach 5 ft., one target. *Hit:* 8 (`1d8 + 4`) piercing damage, or 13 (`2d8 + 4`) piercing damage while enlarged. If the minotaur moved at least 10 feet straight toward the target immediately before it hit, the target takes an extra 4 (`1d8`) piercing damage, or 9 (`2d8`) piercing damage if the minotaur is enlarged. If the target is a creature, it must succeed on a DC 14 Strength saving throw or be pushed up to 10 feet from the minotaur and have the [prone](5E2014官方资源/规则/conditions.md#prone) condition.

***Mattock.*** *Melee Weapon Attack:* `+6` to hit, reach 10 ft., one target. *Hit:* 10 (`1d12 + 4`) bludgeoning damage, or 17 (`2d12 + 4`) bludgeoning damage while enlarged. If the target is a creature and the minotaur is enlarged, the target must succeed on a DC 14 Strength saving throw or have the [prone](5E2014官方资源/规则/conditions.md#prone) condition.
...
***Unerring Tracker.*** The minotaur magically creates a psychic link with one creature it can see. For the next hour, the minotaur knows the current distance and direction to the target if it is on the same plane of existence. The link ends if the minotaur has the [incapacitated](5E2014官方资源/规则/conditions.md#incapacitated) condition or uses this ability on a different target.
```
^statblock